...
"""
class GPEIChooser(BaseChooser):

    def __init__(self, expt_dir, covar="Matern52", mcmc_iters=10,
                 pending_samples=100, noiseless=False, keep_ei=False):
...
#        self.locker.unlock(self.state_pkl)
    ...
    def _save_cov(self, grid, candidates, ei ):
        pass

    def next(self, grid, values, durations, candidates, pending, complete):

        print 'begin search %.2fMB'%( get_ram_MB())
        ...
        if self.mcmc_iters > 0:
            ...
            avg_ei = np.zeros(cand.shape[0])
#            avg_m = np.zeros(cand.shape[0])
#            avg_v = np.zeros(cand.shape[0])
            
            
            w = 1./self.mcmc_iters
            
            for _mcmc_iter in xrange(self.mcmc_iters):
...
#            self._save_ei(grid, candidates, avg_ei,avg_m, avg_v)

            best_cand = np.argmax(avg_ei)
            ...
            try:
                ...
                print 'silently fails hp optimizer'
                # Initial length scales.
                self.ls = np.ones(self.D)
                # Initial amplitude.
            ...
            print 'before compute_ei : %.2fMB'%( get_ram_MB())
            ei = self.compute_ei(comp, pend, cand, vals)
            print 'after compute_ei : %.2fMB'%( get_ram_MB())
#            self._save_ei(grid, candidates, ei, func_m, func_v)
...
#            if self.keep_ei:
            ...
            print func_m.shape, func_v.shape
