...
#                        print 'Pending count : %d/%d. Searching for another configuration.'%( self.pending_count, self.pool_size )
                    ...
                    with base.Profiler() as choose_profiler:
                        ...
                        chooser_state = self.optimizer.get_chooser_state()
...
#         model_averaging = self.metric.model_averaging
#         for sigma_ratio in [0.2, 0.5, 4.]:
#             self.metric.model_averaging = ArgmaxKDE(sigma_ratio)
#             salad_lossD['kde_%d'%(sigma_ratio*100)] = self._eval_salad( hp_id_N, prob_N )[0] 
#         self.metric.model_averaging = model_averaging
